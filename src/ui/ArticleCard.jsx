...
function ArticleCard({title, category, readingTime, content, url, createdDate, author}) {
    const shortTitle = useTruncateText(title, 50)
    const shortContent = useTruncateText(content, 150)
    return (
        <NavLink className="flex w-full flex-col gap-4" to={url} title={title}>
            <div className="flex w-full">
                <img className="w-full h-auto max-h-80 object-cover" src={aboutMe} alt=""/>
            </div>
